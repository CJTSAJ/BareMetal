...
### 中断调用栈
vfio_pci_ioctl </br>
vfio_pci_set_irqs_ioctl </br>
vfio_pci_set_msi_trigger </br>
...
通过遍历consumers链表内的consumer，比对每一个token是否与producer的token相同，相同则调用connect进行配对，token的值一般为trigger(struct eventfd_ctx *) </br>

__connect(prod, cons)会去更新IRTE表项
